...
void	ft_spritecasting(t_vars *vars, t_data *img, double *buffer)
{
	...
	i = 0;
	//projection and sorting of sprites
	while(i < vars->nSprites)
	{
		...
		//loop through every vertical stripe of sprites on screen
...
void 	ft_floorcasting(t_vars *vars, t_data *img, int w, int h)
{
	vars->x = 1;
    vars->y = 0;
    while (vars->y < h)
...
int render_next_frame(t_vars *vars)
{
    ...
    while(vars->x < w)
    {
      ...
      while (vars->hit == 0)
        ...
        if(vars->map[vars->mapX][vars->mapY] == '1' || vars->map[vars->mapX][vars->mapY] == ' ') 
        {
        	vars->hit = 1;
        }
    }
